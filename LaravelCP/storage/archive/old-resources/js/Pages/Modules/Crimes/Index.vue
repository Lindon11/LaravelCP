...
const remainingCooldown = ref(Math.floor(props.cooldown || 0));

// Countdown timer
let cooldownInterval = null;

// Watch for cooldown prop changes (when page reloads after crime attempt)
watch(() => props.cooldown, (newCooldown) => {
    console.log('Cooldown prop changed:', newCooldown);
    remainingCooldown.value = Math.floor(newCooldown || 0);
    console.log('Updated remainingCooldown:', remainingCooldown.value);
...
onMounted(() => {
    console.log('Crimes page mounted - props.cooldown:', props.cooldown);
    console.log('Crimes page mounted - remainingCooldown.value:', remainingCooldown.value);
...
<template>
    <component :is="embedded ? 'div' : AppLayout" :title="embedded ? undefined : 'Crimes'">
        <div :class="embedded ? '' : 'py-12'">
            <div :class="embedded ? '' : 'max-w-7xl mx-auto sm:px-6 lg:px-8'">
                <!-- Header -->
                ...
                <!-- Cooldown Warning -->
                <div v-if="remainingCooldown > 0" class="bg-yellow-50 border-l-4 border-yellow-500 p-4 mb-6 rounded">
                    <p class="text-yellow-800 font-semibold">⏱️ Cooldown Active</p>
                    <p class="text-yellow-700">You must wait <span class="font-bold text-yellow-900">{{ remainingCooldown }}</span> seconds before committing another crime.</p>
                </div>
